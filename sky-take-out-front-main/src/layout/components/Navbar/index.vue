...
export default class extends Vue {
  ...
  // ---> 在这里新增下面的代码 <---
  get employeeId() {
      // 优先从 Vuex 的 userInfo 中获取
    if (UserModule.userInfo && (UserModule.userInfo as any).id) {
      return (UserModule.userInfo as any).id
    }
    // 如果获取不到，则尝试从 Cookie 中解析
    try {
      const userInfoFromCookie = JSON.parse(Cookies.get('user_info') as any)
      return userInfoFromCookie.id
    } catch (e) {
      return null
    }
  }
